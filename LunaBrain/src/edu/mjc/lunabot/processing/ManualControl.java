...
import edu.mjc.jinput.JInput;
import edu.mjc.lunabot.*;
import edu.mjc.lunabot.util.Location;
import edu.mjc.lunabot.util.Obstacle;
/*
 * Connects to a valid GAMEPAD (xbox controller) using Jinput wrapper
 * and maps inputs to motor control commands sent via serial connection 
 * to an Arduino mega running SerialMotorControl.ino
 *
 */

public class ManualControl extends PApplet{
	...
	public void draw() {
	  ...
	  j1.yP = (int)(map(analogVals[JInput.Y_AXIS], (float)-1.0, (float)1.0, (float)(-j1.theSize), (float)(j1.theSize)));
	  j1.xP = (int)(map(analogVals[JInput.X_AXIS], (float)-1.0, (float)1.0, (float)(-j1.theSize), (float)(j1.theSize)));
	  x = j1.mapX();
	  y = j1.mapY();
	  j1.render();
	  dig.render();
	...
	class Joystick{
	  ...
	  boolean slide;
	  int c, cb;
	  Joystick(int x, int y, int s, int col){
	    xPos = x;
	    yPos = y;
	    theSize = s;
	    xP = 0;
